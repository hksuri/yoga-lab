...
def plot_img_and_mask(img_in, img_out, mask, img_name, dir_output, mode = 'inpainted image'):
    ...
    width = image_back_in_pil.width + image_back_out_pil.width
    height = max(image_back_in_pil.height, image_back_out_pil.height)
    combined_img = Image.new('RGB', (width, height))

    # Place image_back_in on the left
    combined_img.paste(image_back_in_pil, (0, 0))

    # Place image_back_out or its processed form on the right
    combined_img.paste(image_back_out_pil, (image_back_in_pil.width, 0))
