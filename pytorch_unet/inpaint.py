...
# def build_corruption(corrupt_type = 'inpaint-freeform2030'):
#     mask = 'freeform2030'
#     method = build_inpaint_freeform(mask)
#     return method
...
#     # freeform_masks = torch.from_numpy(freeform_masks).to('cuda')
    # return inpaint_freeform

def inpaint_freeform(img, freeform_masks):
        # img: [-1,1]
        n_freeform_masks = freeform_masks.shape[0]
        index = np.random.randint(n_freeform_masks)
        mask = freeform_masks[index].copy()

        zoom_factor = 224 / 256
        # zoom_factor = 1.0
        mask = zoom(mask, (1, zoom_factor, zoom_factor))
